...
function renderFacadeCards(){
 ...
 state.facades.forEach((f,i)=>{
  ...
  const drawBays=()=>{bayList.innerHTML='';f.bays.forEach((b,bi)=>{const p=document.createElement('div');p.className='bay-pill';p.innerHTML=`<select><option ${b===1?'selected':''}>1</option><option value="1.5" ${b===1.5?'selected':''}>1,5</option><option ${b===2?'selected':''}>2</option><option value="2.5" ${b===2.5?'selected':''}>2,5</option><option ${b===3?'selected':''}>3</option></select><span>m</span><button title="Supprimer">×</button>`;p.querySelector('select').addEventListener('change',e=>{f.bays[bi]=Number(e.target.value);update();changed();});p.querySelector('button').addEventListener('click',()=>{f.bays.splice(bi,1);drawBays();update();changed();});bayList.appendChild(p)});};
  card.querySelectorAll('[data-k]').forEach(el=>el.addEventListener('change',e=>{const k=e.target.dataset.k;f[k]=e.target.type==='checkbox'?e.target.checked:(e.target.tagName==='SELECT'?e.target.value:Number(e.target.value));if(k==='pignon')card.querySelector('.pignon-fields').classList.toggle('hidden',!f.pignon);update();changed();}));
...
function draw3D(f,w,h){const xs=bayXs(f),n=levels(f),depth=.8;const P=(x,y,z)=>project(x,y,z,w,h);ctx.fillStyle='#dbe7ef';ctx.beginPath();ctx.moveTo(P(xs[0]-1,0,-1).x,P(xs[0]-1,0,-1).y);ctx.lineTo(P(xs.at(-1)+1,0,-1).x,P(xs.at(-1)+1,0,-1).y);ctx.lineTo(P(xs.at(-1)+1,0,2).x,P(xs.at(-1)+1,0,2).y);ctx.lineTo(P(xs[0]-1,0,2).x,P(xs[0]-1,0,2).y);ctx.closePath();ctx.fill();
